...
export const getSeason = (monthIndex) => {
  ...
  if (month >= 3 && month <= 5) return { name: '春季', icon: '🌸', accent: '🌷', mood: '樱花和嫩芽正在给小岛换新装' };
  if (month >= 6 && month <= 8) return { name: '夏季', icon: '☀️', accent: '🌊', mood: '阳光、海浪和蝉鸣正在营业' };
  if (month >= 9 && month <= 11) return { name: '秋季', icon: '🍄', accent: '🍁', mood: '落叶、蘑菇和温暖灯光很适合写作' };
  return { name: '冬季', icon: '❄️', accent: '⛄', mood: '雪地脚印和热可可陪你慢慢更新' };
};
